---
plan_id: 014
version: 2.3 (spec patch — plan-012 INVALID 박제 (fd64f6c) 후 plan-014 reference sync: §Target band (0.6450→0.65 absolute floor) / §0.5 G-gates G0/G2 / §1.4 join table threshold / §3.2 band classifier / §3.4 G0(d) (plan-012 grep → disclaimer verify) / §3.4 G2 (0.6511→0.65) / §3.4 G3 분기 threshold / §3.5 reference 표 / §0.5 baseline reproduce narrative. §4~§8 STAGE-별 코드 spec 은 c3~c7 commit 에서 채움)
date: 2026-05-14 (Asia/Seoul)
status: spec
based_on:
  - 012
followed_by: []
scope: plan-012 (codebook bake-off + hybrid) 의 5-fold OOF plateau 의 root cause 는 *plan-004 selector + plan-006 F0 의 재사용 강박* — 이 명제를 **premise 로 채택** (본 plan 이 *검증* 하지 않음). 그 premise 위에서 plan-012 paradigm (codebook + classifier + regression hybrid) 을 **재사용 없이 from-scratch 재실험**. 즉 본 plan = "*제대로 된* plan-012 의 재실행" 이지 "재사용이 원인인가 falsify" 가 아님. baseline reproduce 없음 + plan-012 measured 값 reference 없음 (plan-012 result.md = INVALID_REFERENCE 박제 fd64f6c 후 §Target band 은 absolute ≥0.66 / 0.65~0.66 / <0.65 으로 재정의).
exp_ids: []
lb_score: null
---
...
>
> 본 plan 의 OOF 가 §Target band 의 어느 위치 (≥0.66 / 0.65~0.66 / <0.65, absolute) 에 들어가는지가 paradigm 의 진짜 잠재력 측정. plan-012 result.md = INVALID_REFERENCE 박제 (fd64f6c) → measured 값 비교 reference 없음.
>
...
- **Premise**: plan-012 의 plateau 의 root cause = "plan-004 candidate-attention selector 위에 reg_head 만 얹은 minimal patch 가 본 plan 의 task essence ('20% 끌어당김') 와 mismatch" — task essence 에 맞는 *제대로 된 design* 으로 paradigm 잠재력은 0.66+ 까지 살아있음. (plan-012 measured 0.6350 = INVALID_REFERENCE 박제 fd64f6c)
...
- **A baseline (plan-012 minimal-patch reproduce) 별도 실행 안 함**. plan-012 result.md = INVALID_REFERENCE 박제 (fd64f6c) → measured 값 비교 reference 도 X. 본 plan = absolute target band (§Target ≥0.66 / 0.65~0.66 / <0.65) 위에서 paradigm 의 잠재력 measurement 만 박제. baseline reproduce 비용 vs 정보 가치 = 어차피 noise (= INVALID anchor 위 측정).

### Target (judgement criteria) — 절대값 기반, plan-012 INVALID 박제 (fd64f6c) 후 재정의

- **OOF ≥ 0.66** ★ positive band (paradigm 부활): premise 가 옳다는 outcome-level 강한 신호. plan-015 = 본 plan 의 polish + LB 제출. decision-note: 0.66 = competition-level paradigm target (= plan-014 original aspiration, plan-012 INVALID 측정과 무관).
- **0.65 ≤ OOF < 0.66** — partial 회복 band: premise 약하게 지지 + paradigm 자체도 부분 한계. plan-013 의 corrector path 와 비교 필요. plan-015 = corrector + 본 plan 의 hybrid (plan-013 Candidate C 변형) 후보. decision-note: 0.65 = F0 raw hit@1cm 0.6320 (plan-006 hard evidence) + ~0.018 round absolute margin (paradigm 의 minimum 회복 floor — plan-012 INVALID anchor 회피).
- **OOF < 0.65** — negative band: 재사용 끊어도 paradigm 한계. premise 자체 의심. plan-013 의 paradigm 폐기 정당화 + 더 deep path-pivot (`notes/new-ideas.md` KNN/GP/Diffusion 후보).

### G-gates (정량 spec @ §3.4)

- G0: preflight artifact 산출 — F0 init reproduce (±0.005) + anchor 0.01m + soft label entropy ≥ 0.5 nat + plan-012 disclaimer verify [TODO]
...
- G2: 1-fold OOF (fold=0) ≥ 0.65 (informational; <0.60 → severe) [TODO]
...
| c2 | docs | **v2 spec drop — §2 Scope + §3 Pre-registration 채움.** narrative draft → executable spec 졸업. §2.1 In-scope (single path config: E0b Frenet-7Way / shared biGRU / learnable F0 / anchor 0.01m / Gaussian soft label / Adam 50ep) + §2.2 Out-of-scope (selector.py import 0 등 12 항목) + §3.1 fold split (`hash(sample_id, salt='plan-014-v1') % 5` 재구현) + §3.2 metric + §3.3 The Config (단일 path 명세) + §3.4 G-gate 정량 (G0 preflight artifact / G1 재사용 끊김 4가지 + smoke / G2 ≥0.6511 1-fold / G3 5-fold band 분류 / G_final 3파일 sync) + §3.5 plan-012/013 reference. §0.5 G-gates stub → §3.4 link. frontmatter version 1.4→2 + status draft→spec. §4~§8 STAGE-별 코드 spec 은 c3~c7 stub 박제 | [DONE] ad051e2 |
| c2.1 | docs | **v2.1 simplification — §2.2 Out-of-scope negative blacklist 통합.** plan-004/006/012 module·weight 재사용 7 row → 1 row (positive "from-scratch 새 module" 한 줄). §1.5 마지막 bullet 의 redundant negative phrasing 정리. 보존 = §2.1 positive spec (anchor 0.01m / Gaussian soft) / §3.3 The Config / §3.4 G1 import check (measurable G-gate) / 7-mode 표 / §1.2~§1.3 trap chain (historical reference). frontmatter version 2→2.1 | [DONE] 0a3c317 |
| c2.2 | docs | **v2.2 spec patch — GPU device 명시.** §2.1 In-scope Training row + §3.3 The Config Training bullet 에 `device=cuda` (GPU 사용 가능, PyTorch native, plan-012 c18 ff1e578 GPU spec-faithful rerun 인프라 재사용, `torch.cuda.is_available() == True` 가정) 박제. frontmatter version 2.1→2.2 | [DONE] 90d9e0d |
| c2.3 | docs | **v2.3 spec patch — plan-012 INVALID 박제 (fd64f6c) 후 reference sync.** plan-014 의 plan-012 reference 처리: §Target band (0.6450→0.65 absolute floor) / §0.5 G-gates G0 (plan-012 grep → disclaimer verify) + G2 (0.6511→0.65) / §1.4 join table threshold / §3.2 band classifier / §3.4 G0(d)/G2 criterion/G3 분기 threshold / §3.5 reference 표 (plan-012 row 제거, plan-006 F0 + plan-013 G1 만 보존) / §0.5 A baseline reproduce narrative 동기화. decision-note: 0.65 = F0 raw 0.6320 + ~0.018 round absolute margin (plan-012 anchor 회피). frontmatter version 2.2→2.3 | [DONE] b6bf927 |
| c3 | code+exp | STAGE 0 (G0) — `analysis/plan-014/preflight.py` + `preflight.json` 산출. F0 init reproduce + anchor scale 박제 + soft label entropy check + plan-012 disclaimer verify (status `INVALID_REFERENCE` + `disclaimer:` field 박제 grep). spec @ §4 | [TODO] |
...
| c5 | exp | STAGE 2 (G2) — `analysis/plan-014/g2_1fold.py` + `g2_1fold.json` + `runs/baseline/plan014_g2_1fold/`. fold=0 학습 + hit@1cm 측정 + ≥0.65 check (informational, §3.2 partial band floor). spec @ §6 | [TODO] |
...
| baseline | A reproduce (in-plan 측정) | **baseline reproduce 없음 + plan-012 measured 값 reference 없음 (INVALID 박제 fd64f6c)** |
| Target | B − A gap 기반 (≥ +0.020 / +0.005 / < +0.005) | **OOF 절대값 기반 (≥ 0.66 / 0.65~0.66 / < 0.65, absolute — v2.3 sync)** |
...
| < 0.68 | < 0.65 (negative) | 둘 다 실패 — 더 deep path-pivot (`notes/new-ideas.md` KNN/GP/Diffusion 후보) |
| 임의 | 0.65 ≤ OOF < 0.66 (partial) | plan-013 corrector + 본 plan hybrid 의 합체 (plan-013 Candidate C 변형) — plan-015 default |
...
- **negative band 해석의 한계**: §Target negative band (OOF < 0.65) 가 나와도 "premise 오류" 와 "paradigm 자체 한계" 분리 불가 — plan-013 join 필수 (§1.4).
...
- **외부 reference 정책**: plan-012 result.md = INVALID_REFERENCE 박제 (fd64f6c) → measured 값 reference 없음. F0 raw 0.6320 (plan-006 hard evidence) + plan-013 G1 fallback 0.6381 (ref-only) 만 외부 reference. baseline reproduce 없음 → 본 plan 의 *모든 compute 는 B path 에 집중*.
- **plan-004 참조 범위 = 2가지만 (★ v1.3 narrative anchor, §0.5 박제)**: (a) input feature 가공 방식, (b) F0 64%/84% sample cover 입증. 그 외 = 본 plan 의 새 module (`src/pb_0_6822/plan014_paradigm.py`) 안에서 from-scratch build (시계열 모듈도 표준 layer 직접 생성). 코드 (§2 spec 단계) + plan body 양쪽 모두 이 narrative 를 유지.
...
| Validation | **5-fold OOF**, fold = `hash(sample_id, salt='plan-014-v1') % 5` (새 module 내 직접 재구현) | §0.5 G3 spec. decision-note: plan-004 `stable_fold_id` 와 fold 분할 결과 *다를 수 있음* — but plan-014 의 measurement 는 plan-014 scheme 내 self-consistent (외부 비교는 §3.5 reference 표 only; plan-012 measured 값 = INVALID 박제 fd64f6c 후 reference X) |
| Training | Adam, lr=1e-3, batch=256, epochs=50, early-stop patience=5 (val loss), **device=cuda (GPU 사용)** | plan-012 spec reuse (조정 = plan-015 후보). decision-note: plan-012 c18 (ff1e578) GPU spec-faithful rerun 인프라 그대로 — `torch.cuda.is_available() == True` 환경 가정 |
...
| Target | 5-fold OOF band (≥0.66 positive / 0.65~0.66 partial / <0.65 negative, absolute) | §0.5 §Target band 그대로 (v2.3 sync: 0.6450 → 0.65 absolute) |
...
| 항목 | 이유 |
|---|---|
| plan-004/006/012 의 module·weight 재사용 일체 (`selector.py` 함수/클래스, `CandidateAttentionGRUSelector` 통째·부분, plan-004 pretrained GRU weight, plan-006 numpy `frenet_par120_perp_neg020`, `ring_classifier.py` 함수·helper) | §0.5 plan-004 참조 2가지 (a input 가공 / b F0 cover 입증) *외* totality 제거 — 본 plan = 새 module `src/pb_0_6822/plan014_paradigm.py` 안에서 from-scratch build (시계열 모듈도 `nn.GRU` 등 표준 layer 직접 생성). anchor 0.005m / hard label CE 답습 = §2.1 positive spec (C3 0.01m / C4 Gaussian σ=0.01m soft) 으로 implicit 박제 — F2/F4/F5/F7 inversion 일괄 |
...
| Baseline reproduce (plan-012 minimal-patch 재실행) | §0.5 박제 — plan-012 result.md INVALID_REFERENCE 박제 (fd64f6c) 후 measured 값 reference 없음, 본 plan 의 §Target band = absolute (≥0.66 / 0.65~0.66 / <0.65) |
...
- decision-note: plan-004 `stable_fold_id` 와 분할 *다를 수 있음* — 외부 비교 reference 는 §3.5 표 only (plan-012 measured = INVALID 박제 fd64f6c 후 reference 없음, §Target band = absolute)
...
- **band classifier** (§0.5 박제):
  ...
  - 0.65 ≤ OOF < 0.66 → **partial 회복 band**
  - OOF < 0.65 → **negative band** (premise 의심)
...
- **Training**: Adam lr=1e-3, batch=256, epochs=50, patience=5 (val loss), seed=20260514, **device=cuda** (GPU 사용, PyTorch native — plan-012 c18 GPU rerun 인프라 재사용; `torch.cuda.is_available() == True` 환경 가정)
...
- **(d) plan-012 disclaimer verify**: `plans/plan-012-frenet-ring-classification.results.md` 에서 `status: G_final_complete_INVALID_REFERENCE` + `disclaimer:` field 박제 확인 (= plan-012 측정값 reference 금지 명시 verify, 값 자체는 reference 안 함)
...
- **criterion**: 1-fold OOF (fold=0) hit@1cm ≥ **0.65** (= §Target partial band floor, absolute — plan-012 INVALID anchor 회피)
- **fold mapping 가정**: 1-fold 0.65 ≈ 5-fold 0.64~0.66 (fold noise 1폭) → G2 통과 = §Target partial 회복 band 진입 후보
- **fail trigger**: hit@1cm < 0.60 → `g2_severe_underperform` severe (= 구현 버그 의심; 0.60 = "F0 raw hit 0.6320 보다 *낮음* = forward path 망가짐"의 conservative threshold). 0.60 ≤ x < 0.65 → informational continue (G3 진행)
...
- **band-별 분기 (informational, fail trigger 없음)**:
  ...
  - partial (0.65~0.66): plan-015 = corrector + 본 plan hybrid (plan-013 Candidate C 변형)
  - negative (<0.65): plan-015 = deep path pivot (`notes/new-ideas.md` KNN/GP/Diffusion 후보)
...
|---|---|---|
| F0 raw hit@1cm | 0.6320 (plan-006 F0 공식 자체의 hard evidence — §1.5 plan-004 참조 (b) sample cover 측정) | G0 (a) reproduce (±0.005) |
| 5-fold OOF hit@1cm | plan-013 G1 fallback 0.6381 (paradigm 폐기 path 의 best, ref-only) | band 측정 (§3.2): ≥0.66 / 0.65~0.66 / <0.65 |
| 1-fold OOF (fold=0) | — | G2 ≥ 0.65 (informational, §3.2 partial band floor align) |

decision-note: plan-012 measured 값 (0.6350 / 0.6411) 은 INVALID_REFERENCE 박제 (fd64f6c) 후 reference 제거. F0 raw 0.6320 는 plan-006 의 F0 공식 자체의 hard evidence 로서 reference 정당 (= §1.5 plan-004 참조 (b)).
