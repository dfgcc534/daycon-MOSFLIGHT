---
plan_id: 014
version: 3 (spec replacement — v2.x single-path 폐기 후 plan-012 5-Phase frame import. baseline = 4 컴포넌트 from-scratch fixed (재사용 끊기 spirit 보존) / 그 위에서 Phase 1 codebook bake-off (E0a/E0b/E0c) + Phase 2 axis ablation 5 (E1~E5) + Phase 3 aux ablation 3 (E6~E8) + Phase 4 final 5-fold + best stack. single measurement → multi-config measurement.)
...
- **Baseline = 4 컴포넌트 from-scratch fixed** (재사용 끊기 spirit 보존, ablation 대상 *아님*):
  ...
  - **C2 F0**: learnable `(α_par, α_perp_t, α_perp_n)`, init = plan-006 (1.20, −0.20, 0.0).
...
- `hybrid_pred = F0_pred + Σ_{k=0..K−1} prob_k × (a_k + reg_offset_k)`
  - `F0_pred = α_par × F0_par + α_perp_t × F0_perp_t + α_perp_n × F0_perp_n` (3 learnable scalar)
...
- (a) F0 init reproduce: `α=(1.20, −0.20, 0.0)` → 모든 train sample hit@1cm 0.6320 ± 0.005
