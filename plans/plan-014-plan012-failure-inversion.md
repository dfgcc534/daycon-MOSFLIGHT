---
plan_id: 014
version: 1 (draft — narrative re-framed: 검증 → premise-기반 재설계)
...
- **재사용 끊기 — 4 컴포넌트 from-scratch** (= 본 plan 이 *교체하는* 것):
  - **C1 encoder**: classifier 와 regression head 의 표현 분리 — plan-004 `CandidateAttentionGRUSelector` reuse 폐기. 양자택일 (별도 encoder vs 별도 projection branch) 은 §2 spec 단계에서 결정. 별도 encoder = F1/F4 의 full inversion / 별도 projection branch = encoder 본체 공유 → F4 partial inversion. 후자 채택 시 self-label "재사용 끊기" 강도 약화 → §2 채움 단계에서 trade-off 명시 필요.
...
| c1.1 | docs | v1 narrative re-frame — "재사용 원인 검증" → "재사용 전제 + plan-012 재실험" (★ premise 채택, head-to-head 폐기, baseline reproduce 폐기, target 절대값 기반) | [TODO] |
