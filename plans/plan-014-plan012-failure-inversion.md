---
plan_id: 014
version: 4.2 (spec patch — plan-review-master iter 2 fix 8건. (1) §2.1.A C1 row: BiGRU forward output reduction 명시 = `output[:, -1, :]` last-step bidir concat. (2) §5.2 Plan014HybridHead: forward (training) vs hybrid_predict (inference) 두 method 분리 의도 + F0_pred detach 책임 = caller (train_one_fold) 명시. (3) §5.4 G1 smoke (a): initial_val_hit 측정 시점 = model.eval() + Adam.step()=0 random-init forward, threshold = `val_hit_after >= val_hit_initial − 0.05` (절대 hit rate, false-positive margin). (4) §2.1.B.1 E2 K density: dom/second/third frame 선택 룰 박제 — E0a winner=world {x,y,z}, E0b winner=Frenet {t,n,b}, E0c winner=K-Means 재fit (frame 무관). (5) §7.1 E1: anchor variant 표식 추가 (E1a = G2 winner reuse = anchor). (6) §7.1 E4: 2-variable composition 명시 (Huber weight 1.0→0.5 + Hinge 추가, plan-012 hybrid_combined_loss carry). (7) §3.4 G_final registry 6 row 의 12 column 값 룰 박제 (id/status/run_dir/config_path/baseline_id/notes 모두). (8) §5.2 run_kfold_oof config dict key set 명시 + §8.1 E7 LastStep MLP GELU 위치 명시. v4.1 → v4.2.)
