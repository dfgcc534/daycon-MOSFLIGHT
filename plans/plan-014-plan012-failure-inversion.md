...
| c3.2 | docs | **v3.2 spec patch — Input feature 상세.** `make_seq_features` 정의 박제: shape `(N, 6, 9)`, 6 step indices `range(max(3, end_idx-5), end_idx+1)` (pad if <6), per-step 9 dim = `turn_model_features_from_context` 8 dim (speed / prev_speed-ratio / acc_norm-ratio / acc_par-ratio / perp_norm-ratio / jerk_norm-ratio / turn_cos / curvature) + direction 1 dim. source = `src/pb_0_6822/selector.py:280-294 + 406-449` (형식만 reuse, `selector.py` import X). §2.1.A Input pipeline row sync. frontmatter version 3.1→3.2 | [TODO] |
...
| Input pipeline | shape `(N, 6, 9)` 시계열. 6 step indices = `range(max(3, end_idx-5), end_idx+1)` (pad first if <6). per-step 9 dim = `turn_model_features_from_context` output 8 dim ((1) `speed = ‖v_last‖`, (2) `prev_speed / speed`, (3) `acc_norm / speed`, (4) `acc_par_scalar / speed`, (5) `perp_norm / speed`, (6) `jerk_norm / speed`, (7) `turn_cos`, (8) `curvature`) + direction 1 dim. source-of-truth = `src/pb_0_6822/selector.py:280-294 + 406-449` (형식만 reuse, `selector.py` import X — 본 module 안에서 직접 재구현) |
