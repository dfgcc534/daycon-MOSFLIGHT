---
plan_id: 014
version: 1.1 (draft — F4 정확화 spot-fix: "head 공유" → "candidate-attention inductive bias mismatch")
...
|---|---|
| F4 encoder inductive-bias mismatch ★ v1.1 정정 | plan-004 `CandidateAttentionGRUSelector` 는 "trajectory + 27 candidate → 후보 비교" task 에 fit 된 모듈 (cross-attention 핵심 연산). plan-012 의 task = trajectory only → 7 anchor 점수 + 3D offset (candidate 입력 자체 없음). 즉 *입력 형식부터 다른 task* 의 weight·구조를 그대로 끼움 = candidate-attention inductive bias 가 plan-012 task 에 잉여이거나 mismatch. (★ v1.0 의 "두 head 공유" 표현은 misleading — encoder 의 task = "시계열 → 방향 점수" 1개이므로 shared encoder + 2 head 자체는 자연스러움. 진짜 죄목 = "잘못된 task 에 fit 된 모듈을 통째 가져온 것".) |
...
- **재사용 끊기 — 4 컴포넌트 from-scratch** (= 본 plan 이 *교체하는* 것):
  - **C1 encoder** (★ v1.1 정정): plan-004 `CandidateAttentionGRUSelector` reuse 폐기 — plan-012 task ("시계열 → 7 anchor 점수 + 3D offset", **candidate 입력 없음**) 에 fit 된 새 encoder 를 from-scratch design. encoder 의 task 는 본질적으로 "시계열 → 방향 점수" 1개이므로 shared encoder + 2 head (classifier / regression) 가 자연스러운 default — **encoder 개수 (1 vs 2) 는 핵심 아님**. 핵심 = (a) plan-004 의 candidate-attention inductive bias 제거 (cross-attention to candidate tokens 구조 폐기), (b) plan-012 의 입력 형식 (trajectory only) 과 출력 형식 (7 anchor logit + 3D offset) 에 직접 맞춘 arch. 후보 arch (예: pure GRU 또는 Transformer over trajectory + 2 linear head) 는 §2 spec 단계에서 결정. ※ v1.0 narrative 의 "별도 encoder vs 별도 projection branch 양자택일" 은 head 공유 문제로 오진한 결과물 — 폐기.
...
"plan-004 selector reuse"  ─┐
                            │
                            ▶ candidate-attention inductive bias mismatch (F4, v1.1 정정) —
                              plan-004 task = trajectory + 27 candidate → 후보 비교 vs
                              plan-012 task = trajectory only → 7 anchor 점수. 입력 형식이
                              다른 모듈을 통째 가져와 candidate-attention 구조·weight 가
                              plan-012 task 에 잉여·mismatch. (※ "두 head 공유" 가 아니라
                              "잘못된 task 의 모듈 통째 재사용" 이 진짜 죄목.)
                            │
