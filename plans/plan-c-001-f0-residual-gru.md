...
| 입력 feature | seq **12채널** (build_seq_t3 9 + L2 F0(perp=0)-잔차 3, t+2 무관측 step zero-pad) + scalar **48d** (build_scalar_40d 40 + f0_conf 2 + EWMA(0.3) F0-잔차 3 + STA/LTA 3) |
| **F0-자기진단 feature (신규, §1.3)** | F0 의 per-step fit 잔차 `raw(t+2)−f0_perp0(t)` 계열. **고정 공식이라 sample 비교가능한 "F0-법칙 위배=maneuver 강도" 신호** (Kalman 이면 innovation = §1.2 제외 부산물). 전부 anchor-invariant(axis 4/5) + 관측창 산출(leakage 무) + f0_perp0 주입(baseline 일관) |
...
| c4 smoke | §5 `tests/test_plan_c001_smoke.py` — import + 1f1s1e finite + 잔차/복원 round-trip assert (f0_pred + inverse_rotate(rotate(y−f0)) == y) + F0-잔차 leakage assert + W-aux gradient 0 assert | [TODO] |
...
- **F0(고정 공식)이라 넣을 수 있는 이유**: F0 는 *per-sample 적응이 없는 보편 법칙*. per-step 잔차 `raw(t+2)−f0_perp0(t)` 는 **"이 샘플이 F0 운동법칙을 시점별로 얼마나 위배하나" = sample 간 비교가능한 maneuver/non-ballistic 강도** 신호. 관측창에서만 산출(leakage 무).
...
|---|---|---|---|
| **L2 F0-잔차 seq** | 7step×3 → (N,11,3) zero-pad, **GRU seq +3채널** | step별 `raw(t+2)−f0_perp0(t)` yaw 회전 3축 | `plan-021/_build_L2_L4` (f0_perp0 주입, Frenet→yaw) |
...
- **attribution (deferred/optional)**: FR001 이 2변경(swap+feature)이라, movement 시 `--f0-resid-feats` on/off 1-fold ablation 으로 (i) baseline-swap 효과 vs (ii) F0-자기진단 feature 효과 분리. 추가로 floor 차(Kalman vs F0 perp0)도 분해. **CV-LB 괴리로 OOF attribution 은 약한 신호** 명시.
...
- `f0_resid_feats(X, theta)` → dict: `{seq_resid (N,11,3), f0_conf (N,2), ewma (N,3), sta_lta (N,3)}`. 전부 `f0_perp0` 주입 (baseline 일관, §1.3).
  - **L2 seq_resid**: `plan-021/_build_L2_L4` 로직 재사용하되 (a) `f0_baseline_fn=f0_perp0`, (b) L4(soft-hit) 미산출, (c) Frenet R_wfn 회전 → **KR002 yaw `rotate_xy(·, theta)`** 로 교체. 7 valid step (raw(t+2)≤t_obs) → (N,11,3) zero-pad align (무관측 step 0). theta = `yaw_angle(raw v_last)` (= run_oof 전달).
  - **f0_conf**: `cand_builder` A8 동일 — [f0_perp0 마지막 잔차 norm, step-speed std] (N,2).
  - **ewma / sta_lta**: per-step yaw-rotated F0 잔차의 EWMA(α=0.3) (N,3), EWMA(0.5)/EWMA(0.1) 비 (N,3). cand_builder ctx base/A1 동일 로직.
  - **leakage 안전**: step t 의 잔차는 sub-window `X[:, t-4:t-1]` 로 +2 예측 → raw(t) 와 비교 (raw(t+2) 미참조). 모든 산출 관측창 내. smoke 에서 assert.
