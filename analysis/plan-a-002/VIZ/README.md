...
### `kr008_move_distance.png` (2×2) — 모델이 실제 이동시킨 거리
- **(a)** 이동거리 |pred−kalman| 히스토그램, hit vs miss 분리 — miss 가 더 많이 이동(어려운 샘플, 잔차 큼).
- **(b)** yaw frame 축별 이동량 — along-motion(평균 0.29cm)이 lateral(0.12cm)의 2.5배, vertical z 0.19cm. 보정은 주로 진행방향 거리.
- **(c)** 이동거리 ECDF (all/hit/miss) — 93%가 <1cm, 2cm 이상은 0.5%.
- **(d)** 이동거리 vs 오차감소(|km−y|−|pred−y|) — 0 위 = 보정이 도움. 작은 이동이 대부분 도움, 큰 이동(캡 근처)은 효과 혼재.

이동거리 요약: mean 0.41cm / median 0.31cm / p90 0.82cm / max 2.51cm (대각 캡 2.83cm 미달).
