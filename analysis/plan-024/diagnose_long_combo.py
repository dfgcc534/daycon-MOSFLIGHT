"""plan-024 combo — poss 1 (hidden 128) + poss 3 (input Gaussian noise aug).

new worktree: worktree-plan-024-combo (사용자 명시 isolation).

setup:
  - hidden 128 (poss 1, capacity sweet spot, 273K params)
  - input Gaussian noise σ=0.05×std (poss 3, augmentation)
  - epoch 100, constant lr 5e-4
  - 1 fold (fold 0) + best epoch tracking

목표: poss 1 의 last-epoch 학습 가능성 (h128 overfit 늦게) + poss 3 의 aug 효과
  → plan-022 carry 0.6528 도달 또는 ↑ 가능성.
"""
...
def main():
    ...
    print(f"[combo h128+aug] hidden={HIDDEN}, noise_σ={NOISE_SIGMA_RATIO}×std, "
    ...
    seq_std = torch.from_numpy(seq_tr.std(axis=(0, 1)).astype(np.float32))
    cand_std = torch.from_numpy(cand_tr.std(axis=(0, 1)).astype(np.float32))
    ...
    print(f"[combo] params={n_params:,}", flush=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % LOG_EVERY == 0 or epoch == EPOCHS - 1:
            ...
            print(f"[combo] ep {epoch:3d}: train={train_loss:.4f} val={val_loss:.4f} "
    ...
    out = {
        "config": {"hidden": HIDDEN, "epochs": EPOCHS, "lr": LR,
                   "noise_sigma_ratio": NOISE_SIGMA_RATIO, "scheduler": "constant"},
        "n_params": n_params,
    ...
    out_path = _THIS / "diagnose_long_combo.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"\n[combo] best hit_1cm={best_hit_1cm:.4f} @ epoch {best_epoch}", flush=True)
    print(f"[combo] vs plan-022 0.6528: {best_hit_1cm - 0.6528:+.4f}", flush=True)
    print(f"[combo] vs poss 3 0.6505: {best_hit_1cm - 0.6505:+.4f}", flush=True)
    print(f"[combo] total {time.time() - t0:.0f}s → {out_path}", flush=True)
