"""plan-024 3-way 5-fold OOF re-evaluation — poss 1/2/3 모두 정직 평가.

직전 1-fold 만 측정 → 5-fold OOF (patience 10 + best val_loss state) 로 재측정.

configs:
  poss 1: hidden 128, NO aug, no strong reg
  poss 2: hidden 384, NO aug, STRONG reg (drop 0.5/0.3 + wd 0.1)
  poss 3: hidden 384, aug σ=0.05, no strong reg

각 5-fold OOF, epoch 100 max + patience 10 (best val_loss state).
"""
...
CONFIGS = [
    {"name": "poss1_h128_noaug", "hidden": 128, "noise": 0.0, "drop_p": 0.3, "seq_drop_p": 0.2, "wd": 0.02},
    {"name": "poss2_strong_reg", "hidden": 384, "noise": 0.0, "drop_p": 0.5, "seq_drop_p": 0.3, "wd": 0.10},
    {"name": "poss3_h384_aug", "hidden": 384, "noise": 0.05, "drop_p": 0.3, "seq_drop_p": 0.2, "wd": 0.02},
]


def train_fold(cfg, seq_tr_full, cand_tr_full, q_tr_full, seq_va, cand_va, seed=20260521):
    ...
    model = model_mod.CrossAttentionAnchorSelector(
        hidden=cfg["hidden"], cand_drop_p=cfg["drop_p"], seq_drop_p=cfg["seq_drop_p"],
    )
    optim = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=cfg["wd"])
    ...
    for epoch in range(EPOCHS):
        ...
        for s_idx in range(0, val_split, BATCH):
            ...
            if cfg["noise"] > 0:
                seq_b = seq_b + torch.randn_like(seq_b) * (cfg["noise"] * seq_std[None, None, :])
                cand_b = cand_b + torch.randn_like(cand_b) * (cfg["noise"] * cand_std[None, None, :])
...
def run_5fold(cfg, X, Y, ids, R_wfn_all, pred_F0_world_all, regimes_all, anchors,
              q_true_all, mw_path):
    N = X.shape[0]; K = anchors.shape[0]
    ...
    for k_fold in range(N_FOLDS):
        ...
        q_pred_va, best_val = train_fold(cfg, seq_tr, cand_tr, q_true_tr, seq_va, cand_va)
        ...
        per_fold_hits.append(hit)
        print(f"  [{cfg['name']}] fold {k_fold}: hit={hit:.4f} val={best_val:.4f} {time.time()-t_f:.0f}s", flush=True)
    ...
    return {
        ...
        "per_fold_hits": per_fold_hits,
        "hit_1cm": float((d_cell <= R_HIT).mean()),
        "hit_1.5cm": float((d_cell <= 0.015).mean()),
    }


def main():
    t0 = time.time()
    print(f"[3way 5-fold] {len(CONFIGS)} configs × 5-fold OOF, epoch={EPOCHS} max patience={PATIENCE}", flush=True)
    ...
    results = []
    for cfg in CONFIGS:
        print(f"\n=== {cfg['name']} ===  hidden={cfg['hidden']} noise={cfg['noise']} "
              f"drop={cfg['drop_p']}/{cfg['seq_drop_p']} wd={cfg['wd']}", flush=True)
        res = run_5fold(cfg, X, Y, ids, R_wfn_all, pred_F0_world_all, regimes_all, anchors,
                         q_true_all, mw_path)
        results.append(res)
        print(f"  [{cfg['name']}] OOF hit_1cm={res['hit_1cm']:.4f} hit_1.5cm={res['hit_1.5cm']:.4f}", flush=True)

    out = {"configs": CONFIGS, "results": results, "elapsed_sec": time.time() - t0}
    out_path = _THIS / "results_3way_5fold.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"\n[3way 5-fold] total {time.time()-t0:.0f}s → {out_path}", flush=True)
    print("\n=== SUMMARY ===")
    for r in results:
        print(f"  {r['name']:25s}: hit_1cm={r['hit_1cm']:.4f}  per-fold={[f'{h:.4f}' for h in r['per_fold_hits']]}",
              flush=True)
    print(f"\nplan-022 carry: 0.6528 (reference)", flush=True)
    print(f"combo 5-fold (carry, h128+aug): 0.6377", flush=True)
