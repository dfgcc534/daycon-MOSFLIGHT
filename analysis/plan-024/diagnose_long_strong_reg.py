"""plan-024 가능성 2 — hidden 384 + 강한 regularization + epoch 100.

setup:
  - hidden 384 (v1 default, capacity 충분)
  - cand_drop_p=0.5 / seq_drop_p=0.3 (강한 channel dropout)
  - weight_decay=0.1 (10× v1)
...
def main():
    ...
    print(f"[poss2 strong_reg] hidden={HIDDEN}, drop={CAND_DROP}/{SEQ_DROP}, wd={WD}, "
    ...
    model = model_mod.CrossAttentionAnchorSelector(
        hidden=HIDDEN, cand_drop_p=CAND_DROP, seq_drop_p=SEQ_DROP,
    )
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[poss2] params={n_params:,}", flush=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % LOG_EVERY == 0 or epoch == EPOCHS - 1:
            ...
            print(f"[poss2] ep {epoch:3d}: train={train_loss:.4f} val={val_loss:.4f} "
    ...
    out = {
        "config": {"hidden": HIDDEN, "epochs": EPOCHS, "lr": LR, "wd": WD,
                   "cand_drop": CAND_DROP, "seq_drop": SEQ_DROP},
        "n_params": n_params,
    ...
    out_path = _THIS / "diagnose_long_strong_reg.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"\n[poss2] best hit_1cm={best_hit_1cm:.4f} @ epoch {best_epoch}", flush=True)
    print(f"[poss2] vs plan-022 0.6528: {best_hit_1cm - 0.6528:+.4f}", flush=True)
    print(f"[poss2] total {time.time() - t0:.0f}s → {out_path}", flush=True)
