"""plan-024 가능성 1 — hidden 128 + epoch 50 + constant lr.

목표: hidden=128 (v4 sweet spot) + 충분 학습 시간 + best epoch tracking.
hidden=384 (long-diag) 의 best 0.6495 @ epoch 35 보다 더 좋은 결과 가능 추정.
"""
...
def main():
    ...
    print(f"[poss1 hidden={HIDDEN}] EPOCHS={EPOCHS}, lr={LR} constant, batch={BATCH}", flush=True)
    ...
    print(f"[poss1] params={n_params:,}", flush=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % LOG_EVERY == 0 or epoch == EPOCHS - 1:
            ...
            print(f"[poss1] ep {epoch:3d}: train={train_loss:.4f} val={val_loss:.4f} "
                  f"hit_1cm={hit_1cm:.4f}", flush=True)
            log.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss,
                        "hit_1cm": hit_1cm})

    out = {
        "config": {"hidden": HIDDEN, "epochs": EPOCHS, "lr": LR, "scheduler": "constant"},
        "n_params": n_params,
    ...
    out_path = _THIS / "diagnose_long_hidden128.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"\n[poss1] best hit_1cm={best_hit_1cm:.4f} @ epoch {best_epoch}", flush=True)
    print(f"[poss1] vs plan-022 0.6528: {best_hit_1cm - 0.6528:+.4f}", flush=True)
    print(f"[poss1] total {time.time() - t0:.0f}s → {out_path}", flush=True)
