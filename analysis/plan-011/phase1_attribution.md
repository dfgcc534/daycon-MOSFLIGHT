...
| F3 (per-sample MLP) | 0.0980 | -0.5421 | ★ cand formula 가 selector.make_candidates 와 numerical 불일치 |
| F4 (LearnableSingleCandidate) | 0.0322 | -0.6079 | ★ 동일 — init_coef(1.94, ...) 로 생성한 cand 가 F0 anchor 와 큰 격차 |

F̂ = F0 fix per §8.5 `formula_swap_marginal`. F3/F4 의 cand formula bug 는 plan-011.1 carry-over 1순위.
...
| F̂ | **F0** (anchor) | §8.5 formula_swap_marginal — no swap |
...
- ❌ Phase 3 4 pair *모두 skip* — 0/4 axis positive (strict threshold 미달).
- ✅ G_final 직접 진입 — best Phase = Phase 1 max (= In axis ID at 0.6450, fold-0 OOF).
- ✅ plan-012 carry-over **paradigm 교체** 후보 박제 (Phase 3+ 미진행으로 ceiling 정의 미확정 — 그러나 4-axis breadth 자체가 corrector path 의 *제한* 노출).

> ★ *Phase 3 skip 의 alternate analysis*: P3.1 (L̂+In̂) 만 진행 시 +0.005~0.01 추가 신호 가능성 (informational) — 본 plan 범위 외, plan-012 carry-over.
