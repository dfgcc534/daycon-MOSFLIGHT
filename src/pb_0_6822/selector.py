"""
PB_0.6822 SELECTOR — Attn-GRU Candidate Selector + 27 Physics Candidates + 18×27 Regime Bias

Extracted verbatim from notes/PB_0.6822 코드공유.ipynb cell 4 (plan-004 c2).

NO LOGIC MODIFICATIONS — pure cell→module extraction for src/* convention.
Source: Public Dacon code share, PB 0.6822 ("Algorithm Notes: Physics Ladder").

Exports:
    CANDIDATES: list[CandidateSpec]            # 27 physics formula specs
    SELECTOR_MAIN: callable                     # argparse entry point (= main)
    fit_regime_bins, assign_regimes,
    candidate_regime_bias, candidate_physics_bias
    make_candidates, make_candidate_features
    motion_terms, read_labels, load_stack,
    read_xyz_csv, read_submission_ids,
    stable_fold_id, metrics, R_HIT, EPS
"""
# ruff: noqa
# fmt: off
...
def main() -> None:
    ...
    parser.add_argument("--models", type=str, nargs="*", default=["gru", "tcn"], choices=["gru", "attn_gru", "lidar_gru", "bigru", "lstm", "tcn", "physics_tcn"])
